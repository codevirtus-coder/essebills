...
import { AdminPrimaryButton, AdminStatusBadge } from "./shared/AdminControls";
...
const Support: React.FC = () => {
  const [activeView, setActiveView] = useState<"faqs" | "tickets" | "contact">(
    "faqs",
  );
  ...
  const columns: TableColumn<Ticket>[] = [
    ...
    },
    {
      ...
      render: (ticket) => (
        <div>
          <p className="text-sm font-bold text-dark-text dark:text-white">
            {ticket.subject}
          </p>
          <p className="text-[10px] text-neutral-text uppercase font-black">
            {ticket.category}
          </p>
        </div>
      ),
    ...
    },
    {
      ...
      align: "right",
      render: (ticket) => (
        <span className="text-sm text-neutral-text">{ticket.lastUpdate}</span>
      ),
    },
  ];
  ...
  const handleOpenTicket = () => {
    ...
    setTimeout(() => {
      ...
      alert(
        "Support request initiated. A new ticket will be available in your dashboard shortly.",
      );
    }, 2000);
  };

  return (
    <div className={`${ADMIN_LAYOUT_SHELL} max-w-6xl mx-auto`}>
      <div className="text-center space-y-4 mb-12">
        <h2 className="text-4xl font-black text-dark-text dark:text-white tracking-tight">
          Support Center
        </h2>
        <p className="text-neutral-text max-w-xl mx-auto">
          Find answers to common questions or reach out to our dedicated support
          team for personalized assistance.
        </p>

        <div className="relative max-w-2xl mx-auto pt-4">
          <span className="material-symbols-outlined absolute left-4 top-[calc(50%+8px)] -translate-y-1/2 text-neutral-text">
            search
          </span>
          <input
            type="text"
        ...
        {activeView === "faqs" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in duration-300">
            {faqs.map((faq, i) => (
              <div
                key={i}
                className="bg-white  p-8 rounded-3xl border border-neutral-light dark:border-white/5 hover:border-primary/30 transition-all group"
              >
                ...
                <h3 className="text-lg font-bold text-dark-text dark:text-white mb-3">
                  {faq.q}
                </h3>
        ...
        {activeView === "tickets" && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-dark-text dark:text-white">
                Recent Tickets
              </h3>
              <AdminPrimaryButton
        ...
        {activeView === "contact" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in fade-in duration-300">
            <div className="bg-white  p-10 rounded-[2.5rem] border border-neutral-light dark:border-white/5 flex flex-col items-center text-center group hover:shadow-2xl hover:shadow-primary/5 transition-all">
              ...
              <h4 className="text-xl font-black text-dark-text dark:text-white mb-2">
                Email Support
              </h4>
              <p className="text-sm text-neutral-text mb-6">
                Send us a detailed message. We aim to reply within 4 business
                hours.
              </p>
              <a
            ...
            <div className="bg-white  p-10 rounded-[2.5rem] border border-primary/20 dark:border-white/5 flex flex-col items-center text-center group hover:shadow-2xl hover:shadow-primary/5 transition-all relative">
              <div className="absolute top-6 right-6 px-3 py-1 bg-accent-green text-dark-text text-[9px] font-black rounded uppercase tracking-widest">
                Active Now
              </div>
              ...
              <p className="text-sm text-neutral-text mb-6">
                Chat with one of our support agents in real-time for quick
                resolutions.
              </p>
              <button
                onClick={handleOpenTicket}
            ...
            <div className="bg-white  p-10 rounded-[2.5rem] border border-neutral-light dark:border-white/5 flex flex-col items-center text-center group hover:shadow-2xl hover:shadow-primary/5 transition-all">
              ...
              <a
                ...
                className="text-accent-green font-black uppercase text-xs tracking-widest hover:underline"
              >
                +263 123 456 789
              </a>
            </div>
      ...
      <div className="bg-[#221c35] p-12 rounded-[3rem] text-white flex flex-col md:flex-row items-center justify-between gap-8 mt-12 overflow-hidden relative">
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/60 text-sm max-w-md">
            Looking for API references or integration guides? Visit our
            developer portal for comprehensive documentation.
          </p>
        </div>
