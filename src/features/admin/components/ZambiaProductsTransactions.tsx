...
// Zambia product providers
const ZAMBIA_PRODUCTS = [
  { label: 'ZESCO', slug: 'zesco', endpoint: '/v1/zesco-transactions' },
  { label: 'DSTV', slug: 'dstv', endpoint: '/v1/dstv-transactions' },
  { label: 'GOtv', slug: 'gotv', endpoint: '/v1/gotv-transactions' },
  { label: 'MTN', slug: 'mtn', endpoint: '/v1/mtn-transactions' },
  { label: 'Airtel', slug: 'airtel', endpoint: '/v1/airtel-transactions' },
  { label: 'Zamtel', slug: 'zamtel', endpoint: '/v1/zamtel-transactions' },
  { label: 'Liquid Telecom', slug: 'liquid-telecom', endpoint: '/v1/liquid-telecom-transactions' },
  { label: 'Topstar', slug: 'topstar', endpoint: '/v1/topstar-transactions' },
  { label: 'Madison Life', slug: 'madison-life', endpoint: '/v1/madison-life-transactions' },
  { label: 'Afribus', slug: 'afribus', endpoint: '/v1/afribus-transactions' },
  { label: 'Vodafone', slug: 'vodafone', endpoint: '/v1/vodafone-transactions' },
]
...
const ZambiaProductsTransactions: React.FC = () => {
  const [selectedProduct, setSelectedProduct] = useState(ZAMBIA_PRODUCTS[0].slug)
  ...
  const loadTransactions = useCallback(async () => {
    try {
      setIsLoading(true)
      const product = ZAMBIA_PRODUCTS.find(p => p.slug === selectedProduct)
      if (!product) return
  ...
  const getStatusBadge = (status: string) => {
    ...
    const isSuccess = statusUpper.includes('SUCCESS') || statusUpper.includes('COMPLETED') || statusUpper.includes('CREDITED')
    const isPending = statusUpper.includes('PENDING') || statusUpper.includes('PROCESSING')
  ...
  return (
    <div className="p-8 space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-dark-text dark:text-white">Zambia Products Transactions</h2>
          <p className="text-sm text-neutral-text">View and manage all Zambia-based product transactions.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-neutral-light dark:border-white/5">
          ...
          <p className="text-2xl font-extrabold text-green-600">{rows.filter(r => String(r.status ?? r.transactionStatus ?? '').toUpperCase().includes('SUCCESS')).length}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-neutral-light dark:border-white/5">
          <p className="text-xs font-black text-neutral-text uppercase tracking-wider">Pending</p>
          <p className="text-2xl font-extrabold text-yellow-600">{rows.filter(r => String(r.status ?? r.transactionStatus ?? '').toUpperCase().includes('PENDING')).length}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-neutral-light dark:border-white/5">
...
export default ZambiaProductsTransactions
