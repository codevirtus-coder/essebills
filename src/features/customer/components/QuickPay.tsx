...
function CatCard({
  ...
  return (
    ...
    >
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center ${ICON_BG[colorKey(cat.label, cat.key)] ?? ICON_BG.utilities}`}
      >
        {showLucide ? (
          <Icon
            name={cat.emoji}
      ...
      <p
        className={`text-[9px] sm:text-[10px] font-semibold text-center leading-tight line-clamp-2 w-full px-0.5 transition-colors ${focused ? "text-emerald-800" : "text-slate-600"}`}
      >
        {cat.label}
      </p>
      {count > 0 && (
...
function ProductCard({
  ...
  return (
    ...
    >
      <div
        className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center ${ICON_BG[ck] ?? ICON_BG.utilities}`}
      >
        {showLucide ? (
          <Icon
            name={shownGlyph}
      ...
      <div className="flex flex-col items-center min-w-0">
        <p
          className={`text-[9px] sm:text-[10px] font-semibold text-center leading-tight line-clamp-2 transition-colors ${focused ? "text-emerald-800" : "text-slate-700"}`}
        >
          {biller.name}
        </p>
        {biller.currencyCode && (
    ...
    () =>
      (data?.categories ?? ([] as ProductCategory[]))
        ...
        .map((c) => ({
          ...
          emoji:
            String(c.emoji ?? "").trim() ||
            catEmoji(String(c.displayName ?? c.name ?? "")),
        }))
  ...
  const allProducts = useMemo<BillerCard[]>(
    () =>
      (data?.products ?? [])
        .filter(
          (p) =>
            p.status === "ACTIVE" && !p.deleted && typeof p.id === "number",
        )
        .map((p) => {
          ...
          const catId =
            typeof cat?.id === "number" ? String(cat.id) : undefined;
          return {
            id: `p-${p.id}`,
            ...
            minimumPurchaseAmount:
              Number(p.minimumPurchaseAmount ?? 0) || undefined,
...
export function QuickPaySelector({
  onPick,
  catCols = 3,
  itemCols = 2,
}: SelectorProps) {
  ...
  const visibleProducts = useMemo<BillerCard[]>(() => {
    ...
    if (selectedCat)
      return allProducts.filter((b) => b.categoryKey === selectedCat.key);
    return allProducts;
  ...
  function pickVariant(variant: Product) {
    ...
    pickProduct({
      ...
      productCategoryId:
        typeof cat?.id === "number" ? cat.id : selectedBase.productCategoryId,
      name,
      ...
      currencyCode:
        (variant as any).defaultCurrency?.code ?? selectedBase.currencyCode,
      minimumPurchaseAmount:
        Number(
          (variant as any).minimumPurchaseAmount ??
            selectedBase.minimumPurchaseAmount ??
            0,
        ) || undefined,
  ...
  useEffect(() => {
    if (step !== "variants") searchRef.current?.focus();
  }, [step]);
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    ...
    switch (e.key) {
      ...
      case "ArrowDown":
        e.preventDefault();
        setFocusedIdx((i) => Math.min(i + cols, itemCount - 1));
        break;
      case "ArrowUp":
        e.preventDefault();
      ...
      case "Enter":
        e.preventDefault();
        if (step === "categories") {
          const c = categories[focusedIdx];
          if (c) goToProducts(c);
        } else if (step === "products") {
          const p = visibleProducts[focusedIdx];
          if (p) openVariants(p);
        }
        break;
      case "Escape":
        e.preventDefault();
        if (step !== "categories") goBack();
        break;
    }
  }
  ...
  return (
    <div
      className="flex flex-col h-full outline-none p-3"
      onKeyDown={handleKeyDown}
      tabIndex={-1}
      role="application"
      aria-label="Quick Pay service selector"
    >
      {/* Fixed header */}
          ...
          <div className="space-y-2">
              ...
              <button
                ...
                onClick={() => {
                  const b = allProducts.find(
                    (x) => x.productId === lastSel.productId,
                  );
                  if (b) openVariants(b);
                }}
            ...
            {isLoading ? (
              <div className={`grid ${catColClass} gap-4`}>
                {Array(9)
                  ...
                  .map((_, i) => (
                    <div
                      key={i}
                      className="aspect-square rounded-xl animate-pulse bg-slate-100 flex flex-col items-center justify-center gap-1.5 p-2.5"
                    >
              ...
              <div className="flex flex-col items-center gap-2 py-8 text-slate-300">
                <Zap size={22} />
                <p className="text-xs">No services available.</p>
              </div>
            ) : (
              <div className={`grid ${catColClass} gap-4`}>
              ...
              {visibleProducts.map((b, i) => (
                <ProductCard
                  ...
                  glyph={
                    ...
                    (b.productCategoryId != null
                      ? categoryGlyphByKey[String(b.productCategoryId)]
                      : undefined)
                  }
                  focused={focusedIdx === i}
                  onClick={() => openVariants(b)}
                  itemRef={(el) => {
                    itemRefs.current[i] = el;
                  }}
                />
      ...
      <p className="shrink-0 pt-1 text-[9px] text-slate-300 text-right hidden sm:block">
        ↑↓ ←→ · Enter · Esc · Home · End
      </p>
    </div>
  );
